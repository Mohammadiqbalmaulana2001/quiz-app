...
function Home() {
   ...
   return (
      <div className="bg-zinc-700 rounded-2xl w-2/4 min-h-screen text-slate-50 flex justify-center py-16">
         {!showResult ?
            (
            <div className="flex flex-col space-y-6 w-4/6">
               <h1 className="py-4 rounded-xl bg-zinc-800 text-sky-400 text-3xl font-black cursor-pointer text-center">Quiz App</h1>            
               <div className="flex flex-col space-y-3 bg-zinc-800 rounded-xl p-4">
                  <div className="flex flex-row justify-between border-b border-zinc-800 py-4 text-xl font-bold">
                     <div>Question {(currentQuestion >= 9 ? (currentQuestion + 1) : +(currentQuestion + 1))}<span className="text-sky-400">/</span><span className="text-zinc-400 text-base">{triviaData.length}</span></div>
                     <div>{timer}</div>
                  </div>
